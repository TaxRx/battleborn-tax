...
class DeductionCalculator implements StrategyCalculator {
  calculateImpact(strategy: TaxStrategy, taxInfo: TaxInfo, rates: TaxRates) {
    ...
    const standardOrCustomDeduction = modifiedTaxInfo.standard_deduction ?
      (rates.federal.standard_deduction[modifiedTaxInfo.filing_status] || 0) :
      (modifiedTaxInfo.custom_deduction || 0);
    ...
    if (totalProposedDeductions > maxAllowedDeduction) {
      ...
      modifiedTaxInfo.deduction_limit_reached = true;
    }
