...
// .wrangler/tmp/bundle-ZEIhIO/checked-fetch.js
...
var ms, hasRequiredMs, msExports, humanize, common, hasRequiredCommon, commonExports, setup, createDebug;
var init_debug = __esm({
  "node_modules/@cloudflare/unenv-preset/dist/runtime/npm/debug.mjs"() {
    ...
    msExports = requireMs();
    humanize = /* @__PURE__ */ getDefaultExportFromCjs(msExports);
...
// .wrangler/tmp/bundle-ZEIhIO/middleware-loader.entry.ts
init_checked_fetch();
init_modules_watch_stub();
init_virtual_unenv_global_polyfill_cloudflare_unenv_preset_node_process();
init_virtual_unenv_global_polyfill_cloudflare_unenv_preset_node_console();
...
// .wrangler/tmp/bundle-ZEIhIO/middleware-insertion-facade.js
init_checked_fetch();
init_modules_watch_stub();
init_virtual_unenv_global_polyfill_cloudflare_unenv_preset_node_process();
init_virtual_unenv_global_polyfill_cloudflare_unenv_preset_node_console();
...
var WaitTask = class {
  ...
  async rerun() {
    ...
    if (!error3 && await __classPrivateFieldGet11(this, _WaitTask_isolatedWorld, "f").evaluate((s2) => {
      return !s2;
    }, success).catch(() => {
      return true;
    })) {
  ...
  async move(x, y, options = {}) {
    ...
    for (let i2 = 1; i2 <= steps; i2++) {
      await __classPrivateFieldGet20(this, _Mouse_client, "f").send("Input.dispatchMouseEvent", {
        ...
        x: fromX + (__classPrivateFieldGet20(this, _Mouse_x, "f") - fromX) * (i2 / steps),
        y: fromY + (__classPrivateFieldGet20(this, _Mouse_y, "f") - fromY) * (i2 / steps),
...
__name(log3, "log");
router.get("/health", () => {
  log3("info", "Health check requested");
...
router.post("/api/generate-pdf", async (request, env3) => {
  try {
    log3("info", "PDF generation request received");
    const { html, filename = "document.pdf", options = {} } = await request.json();
    if (!html) {
      log3("error", "No HTML content provided");
    ...
    log3("info", `Generating PDF: ${filename}`, {
    ...
    if (!env3.BROWSER) {
      log3("error", "Browser binding not available");
      return corsResponse(new Response(JSON.stringify({
    ...
    log3("debug", "Launching browser session");
    const browser = await puppeteer_core_default.launch(env3.BROWSER);
    const page = await browser.newPage();
    try {
      await page.setViewport({ width: 1200, height: 1600 });
      log3("debug", "Setting page content");
      await page.setContent(html, {
        waitUntil: ["domcontentloaded", "networkidle0"],
        timeout: 3e4
      });
      log3("debug", "Waiting for fonts and render completion");
      await page.evaluateHandle(() => document.fonts.ready);
      await new Promise((resolve) => setTimeout(resolve, 3e3));
      log3("debug", "Generating PDF with options", pdfOptions);
      const pdfBuffer = await page.pdf(pdfOptions);
      log3("info", `PDF generated successfully`, {
        filename,
      ...
      await page.close();
      await browser.close();
      log3("debug", "Browser session closed");
    }
  } catch (error3) {
    log3("error", "PDF generation failed", {
...
var src_default = {
  async fetch(request, env3, ctx) {
    try {
      globalThis.ENVIRONMENT = env3.ENVIRONMENT || "unknown";
      globalThis.VERSION = env3.VERSION || "1.0.0";
      globalThis.LOG_LEVEL = env3.LOG_LEVEL || "info";
      log3("debug", "Request received", {
      ...
      log3("error", "Unhandled error in main fetch handler", {
        error: error3.message,
        stack: error3.stack
      });
      const response = new Response(JSON.stringify({
