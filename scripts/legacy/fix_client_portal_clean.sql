-- CLEAN CLIENT PORTAL TOKEN SYSTEM FIX
-- This script removes all existing functions and creates clean versions

-- First, drop ALL existing versions of the functions to avoid conflicts
DO $$
BEGIN
  -- Drop all possible versions of generate_portal_token
  DROP FUNCTION IF EXISTS generate_portal_token(UUID) CASCADE;
  DROP FUNCTION IF EXISTS generate_portal_token(UUID, INTEGER) CASCADE;
  DROP FUNCTION IF EXISTS generate_portal_token(UUID, TEXT) CASCADE;
  
  -- Drop all possible versions of validate_portal_token  
  DROP FUNCTION IF EXISTS validate_portal_token(VARCHAR) CASCADE;
  DROP FUNCTION IF EXISTS validate_portal_token(VARCHAR, INET) CASCADE;
  DROP FUNCTION IF EXISTS validate_portal_token(TEXT) CASCADE;
  DROP FUNCTION IF EXISTS validate_portal_token(TEXT, INET) CASCADE;
  
  RAISE NOTICE 'Dropped all existing portal token functions';
END $$;
...
-- Create the generate_portal_token function (clean version)
...
-- Create the validate_portal_token function (clean version)
...
BEGIN
  ...
  IF test_business_id IS NOT NULL THEN
    -- Generate a token using explicit function call
    SELECT token INTO test_token 
    FROM generate_portal_token(test_business_id::UUID);
    
    RAISE NOTICE 'Generated test token: %', SUBSTRING(test_token, 1, 10) || '...';
    
    -- Validate the token using explicit function call
    SELECT * INTO test_result 
    FROM validate_portal_token(test_token::VARCHAR);
