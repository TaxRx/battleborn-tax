...
class CommissionService {
  ...
  async assignExpert(assignmentData: AssignExpertForm, assignedBy: string): Promise<ProposalAssignment> {
    try {
      ...
      await supabase.rpc('increment_expert_workload', { expert_id: assignmentData.expert_id });
  ...
  async getCommissionTransactions(
    ...
    try {
      let query = supabase
        ...
        .select(`
          *,
          affiliate:profiles!affiliate_id(full_name, email, affiliate_code),
          expert:experts(*)
        `);
  ...
  async getCommissionStats(): Promise<CommissionStats> {
    try {
      // This would typically be done with a single complex query or stored procedure
      // For now, we'll make multiple queries and combine the results
      ...
      const [
        ...
        this.getTopPerformingAffiliates(),
      ...
      return {
        ...
        top_performing_affiliates: topAffiliates,
  ...
  async getAffiliateCommissionDashboard(affiliateId: string): Promise<AffiliateCommissionDashboard> {
    try {
      const transactions = await this.getCommissionTransactions(undefined, affiliateId);
      
      const totalEarned = transactions
        .filter(t => t.transaction_type === 'affiliate_payment_sent' && t.status === 'completed')
        .reduce((sum, t) => sum + t.amount, 0);

      const pendingPayments = transactions
        .filter(t => t.transaction_type === 'affiliate_payment_due' && t.status === 'pending')
        .reduce((sum, t) => sum + t.amount, 0);
      ...
      const monthlyEarnings = transactions
        .filter(t => {
          ...
          return transactionDate.getMonth() === currentMonth &&
                 ...
                 t.transaction_type === 'affiliate_payment_sent' &&
                 t.status === 'completed';
        })
        .reduce((sum, t) => sum + t.amount, 0);
      ...
      return {
        ...
        commission_by_strategy: [], // Would need to join with proposals/strategies
        recent_transactions: transactions.slice(0, 10),
        payment_schedule: [], // Would need to calculate based on pending payments
      };
    } catch (error) {
      console.error('Error fetching affiliate commission dashboard:', error);
      return {
        total_earned: 0,
        pending_payments: 0,
        completed_payments: 0,
        monthly_earnings: 0,
        commission_by_strategy: [],
        recent_transactions: [],
        payment_schedule: [],
      };
    }
  ...
  private async getTopPerformingAffiliates(): Promise<Array<{
    affiliate_id: string;
    affiliate_name: string;
    total_commission: number;
    proposal_count: number;
  }>> {
    // This would require a complex query joining multiple tables
    // For now, return empty array - would implement with proper SQL
    return [];
  }
