...
import { LayoutDashboard, Calculator, User, LogOut, Menu, X, FileText, Settings } from 'lucide-react';
    ...
    <>
      ...
      <nav className="space-y-2">
        {navigationItems.map(item => (
          ...
          >
            <item.icon size={20} className={currentView === item.id ? 'text-white' : 'text-white/80'} />
            {isExpanded && (
              <span className={`ml-3 ${currentView === item.id ? 'text-white font-medium' : 'text-white/80'}`}>
                {item.label}
              </span>
            )}
          </button>
