...
begin
  if not exists (
    select 1
    from pg_trigger
    where tgname = 'trg_rd_client_activity_overrides_u'
      and tgrelid = 'rd_client_activity_overrides'::regclass
  ) then
    create trigger trg_rd_client_activity_overrides_u
    before update on rd_client_activity_overrides
...
begin
  if not exists (
    select 1
    from pg_trigger
    where tgname = 'trg_rd_client_step_overrides_u'
      and tgrelid = 'rd_client_step_overrides'::regclass
  ) then
    create trigger trg_rd_client_step_overrides_u
    before update on rd_client_step_overrides
    for each row execute function set_updated_at();
  end if;
end
$$ language plpgsql;
...
begin
  if not exists (
    select 1
    from pg_trigger
    where tgname = 'trg_rd_client_subcomponent_overrides_u'
      and tgrelid = 'rd_client_subcomponent_overrides'::regclass
  ) then
    create trigger trg_rd_client_subcomponent_overrides_u
    before update on rd_client_subcomponent_overrides
    for each row execute function set_updated_at();
  end if;
end
$$ language plpgsql;
